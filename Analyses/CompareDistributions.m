...
while deviation*100 > tolerance,
	...
	if strcmp(tail,'one'),
		quantiles = 1-alphaGlobal(iteration);
	else
		quantiles = [alphaGlobal(iteration)/2 1-(alphaGlobal(iteration)/2)];
	end
    ...
    if strcmp(tail,'two'),
        ...
        significant = differences > repmat(confidenceIntervals(1,:),nShuffles,1);
    end
	...
	if iteration == 1,
		...
		pointwise = confidenceIntervals;
		alphaGlobal(iteration) = 0.003;
	else
		iteration = iteration + 1;
		s = sign(p-alpha);
		if deviation > 0.05,
			alphaGlobal(iteration) = alphaGlobal(iteration-1) - s*deviation*0.00075;
		else
			alphaGlobal(iteration) = alphaGlobal(iteration-1) - s*deviation*0.033;
		end
		if alphaGlobal(iteration) < 0.001,
...
if(iteration > 1)
    ...
    stats.p = pGlobal;
    stats.alpha = alphaGlobal(1:length(pGlobal)); % In some conditions, alphaGlobal contains the value for the next iteration; if so, drop it
end
