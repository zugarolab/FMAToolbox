function [maps,stats] = SurveyFiringMapsScale(units,varargin)

%SurveyFiringMaps - Compute and plot firing maps for all subsessions.
%
%  USAGE
%
%    [maps,stats] = SurveyFiringMapsScale(units,<options>)
%
...
if ~isempty(minv),
	...
	[unused,in] = Threshold(v,'>',minv,'min',5,'max',1);
...
%  			PlotColorMap(map.rate,map.time,'ydir','reverse','bar','off','cutoffs',[0 0.6*max([st.peak 1])],'gamma',2);
			PlotColorMap(map.rate,map.time,'ydir','reverse','bar','off','cutoffs',[0 10],'gamma',2);
			title = titles{i}(14:end);
			SplitTitle([title ' (' int2str(units(j,1)) '-' int2str(units(j,2)) ') Rate max ' int2str(max(st.peak)) ' Hz'],round(150/sqrt(nSubsessions)));
		end
	end
end
