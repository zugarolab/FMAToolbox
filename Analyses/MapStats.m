...
lambda_i = map.z;
lambda = lambda_i(:)'*p_i(:);
% When firing rate is very low, lambda_i can often be zero, which produces a
% log2(lambda_i/lambda) = -Inf. To avoid this, every instance of 0 is
% modified to eps(0). It was also checked that: x * ln(x) --> 0 
lambda_i(lambda_i==0) = eps(0);
if T == 0 || lambda == 0,
	stats.specificity = 0;
else
...
if strcmp(localMax,'on');
	% A field should peak at a local maximum, otherwise we can falsely detect a valley between two fields as a field,
	% after detecting and excluding the two surrounding peaks
	localMax = FindLocalMaxima([(1:numel(z))' z(:)]);
else
	% No criterion; any point will do
	localMax = 1:numel(z);
end
...
while true,
	...
	fieldSize = sum(field(:));
	% Keep this field if its size is sufficient (and optionally if it is a local maximum)
	if fieldSize > minSize && ismember(idx,localMax),
		stats.field(:,:,i) = field;
