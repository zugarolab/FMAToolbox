	...
	switch(lower(varargin{i})),
	    ...
	    case 'chunksize',
			...
			if ~isivector(chunkSize,'>0'),
				error('Incorrect value for property ''chunkSize'' (type ''help <a href="matlab:help LoadBinary">LoadBinary</a>'' for details).');
	        end
		case 'precision',
...
% Preallocate memory
data = zeros(ceil(sum(nRecords)/(skip+1)),length(channels));
...
for k = 1:length(dataOffset),
	...
	if nSamples <= chunkSize,
		...
		nSamplesPerChunk = floor(chunkSize/nChannels)*nChannels;
