function [templates,correlations,weights] = ActivityTemplates(spikes,varargin)

%ActivityTemplates - Compute activity templates using PCA (and optionally ICA) of spike trains.
%
...
%  USAGE
%
%    [templates,correlations,weights] = ActivityTemplates(spikes,<options>)
%
%    =========================================================================
%     Properties    Values
%    -------------------------------------------------------------------------
...
%    weights        significant eigenvectors (for PCA), or weights (for ICA)


%
% Time bins can be automatically determined
% using a fixed bin size, or provided as an explicit list (e.g. computed using
% theta phases).
%
%  USAGE
%
%    [templates,correlations,weights,nSignNeurons] = ActivityTemplatesICA(spikes,<options>)
%
%    =========================================================================
%     Properties    Values
%    -------------------------------------------------------------------------
...
%  OUTPUT
%
%    activity				activity pf assemblies for each time bin
%    weights			  cell weights from PCA for each assemblly
...
if nargin < 1,
	error('Incorrect number of parameters (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).');
end
% Check parameter sizes
if ~isdmatrix(spikes,'@2'),
	error('Parameter ''spikes'' is not a Nx2 matrix (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).');
end

% Parse parameter list
for i = 1:2:length(varargin),
	if ~ischar(varargin{i}),
		error(['Parameter ' num2str(i+2) ' is not a property (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).']);
	end
	switch(lower(varargin{i})),
		case 'binsize',
			binSize = varargin{i+1};
			if ~isdscalar(binSize,'>0'),
				error('Incorrect value for property ''binSize'' (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).');
			end
		case 'step',
			step = varargin{i+1};
			if ~isdscalar(step,'>0'),
				error('Incorrect value for property ''step'' (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).');
			end
		case 'bins',
			bins = varargin{i+1};
			if ~isdmatrix(bins,'@2'),
				error('Incorrect value for property ''bins'' (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).');
			end
		case 'mode',
			mode = varargin{i+1};
		...
		otherwise,
			error(['Unknown property ''' num2str(varargin{i}) ''' (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).']);
	end
end

% Options binSize and bins are incompatible
if ~isempty(binSize) && ~isempty(bins),
	error('Parameters ''binSize'' and ''bins'' are incompatible (type ''help <a href="matlab:help ActivityTemplatesICA">ActivityTemplatesICA</a>'' for details).');
end
if isempty(binSize) && isempty(bins),
...
for i=1:size(weights,2),nor(1,i)=norm(weights(:,i)); end
weights = bsxfun(@rdivide,weights,nor);

% Order them by sum of absolute values (arbitrary, just for consistency's sake)
[~,order] = sort(sum(abs(weights)));
