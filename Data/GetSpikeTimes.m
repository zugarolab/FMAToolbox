...
elseif ~isempty(session)
    ...
    load(file_root+'/'+basename+'.cell_metrics.cellinfo.mat','cell_metrics')
    spike_times = vertcat(cell_metrics.spikes.times{:});
    n_spikes_per_unit = cellfun(@(x) size(x,1),cell_metrics.spikes.times).';
    unit_channel = repelem(cell_metrics.shankID.',n_spikes_per_unit,1);
    unit_cluster = repelem(cell_metrics.cluID.',n_spikes_per_unit,1);
    spikes = [spike_times,unit_channel,unit_cluster];
else
    global DATA;
    if isempty(DATA)
...
% EXTRA CONTROL TO REMOVE
global DATA;
if ~isempty(session) && ~isempty(DATA)
    disp("Control result: " + string(spikes == DATA.spikes));
end
