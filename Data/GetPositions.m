...
%     'DAR'          video display aspect ratio (x/y, for non-square pixels)
...
% Copyright (C) 2004-2023 by Michaël Zugaro
%
...
for i = 1:2:length(varargin),
	...
	switch(lower(varargin{i})),
		...
		case 'dar',
			dar = varargin{i+1};
			if ~isdscalar(dar,'>0'),
				error('Incorrect value for property ''DAR'' (type ''help <a href="matlab:help GetPositions">GetPositions</a>'' for details).');
			end
		case 'discard',
...
dar = GetCustomDefaults(dar,'dar',1);
...
% Apply DAR correction for non-square pixels
if dar ~= 1,
	positions(:,2:2:end) = positions(:,2:2:end) * dar;
end
