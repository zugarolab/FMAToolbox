function ax = PiecewiseLinearAxis(x,axis,varargin)

%PiecewiseLinearAxis - Label axis using piecewise linear values.
%
%  USAGE
%
%    ax = PiecewiseLinearAxis(x,axis,<options>)
%
%    x                axis values
...
%     Properties      Values
%    -------------------------------------------------------------------------
%     'ax'            axes to operate on (default = gca)
%     <plot options>  any other property (and all properties that follow)
%                     will be passed to <a href="matlab:help xline">plot</a>

% Copyright (C) 2015 by Michaël Zugaro, 2026 by Pietro Bozzo (options)
%
...
if isastring(varargin{1},'ax') && numel(varargin) > 1
    ax = varargin{2};
...
set(ax,[axis 'lim'],[min(x)-binSize/2 max(x)+binSize/2]);
...
if strcmp(axis,'x')
    xline(ax,X(gaps),varargin{:})
else
    yline(ax,X(gaps),varargin{:})
end
...
set(ax,[axis 'tick'],ticks,[axis 'TickLabel'],tickLabels);